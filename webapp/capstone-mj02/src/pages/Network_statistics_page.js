...
function Networkstatisticspage (){
    ...
    }, [device, ogNetworkDatas]);
    //console.log(device);
    //console.log(ogNetworkDatas);
    useEffect(() => {
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              <div>
                <select defaultValue="" onChange= {e =>setClientSelection(e.target.value)}>
                  ...
                  <option value="3C:61:05:D5:0A:9E">Device 1 (3C:61:05:D5:0A:9E)</option>
              ...
              <div> 
              <h1 class="text-2xl font-semibold mb-6 text-center">Device Statistics</h1>
              <div class="grid grid-cols-2 gap-6">
  <div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-lg font-semibold mb-4">Network Bandwidth Usage</h2>
    {client_selection !== "" && <p class="text-gray-600">2.4 GHz</p>}
  </div>
  <div class="bg-white rounded-lg shadow-md p-6">
  ...
  <div class="bg-white rounded-lg shadow-md p-6">
    ...
    {client_selection !== "" && (
      <p class="text-gray-600">{ogNetworkDatas[ogNetworkDatas.length - 1]["devices"][client_selection]["connected time"]}</p>
    )}
  </div>
</div>
</div>
...
<div>
<h1 class="text-2xl font-semibold mb-6 text-center">Router Statistics</h1>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      {client_selection !== "" && (
        <p class="text-gray-600">
          {ogNetworkDatas[ogNetworkDatas.length - 1]["used_disk"]}/{ogNetworkDatas[ogNetworkDatas.length - 1]["total_disk"]} MB
        </p>
      )}
    </div>
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      {client_selection !== "" && (
        <p class="text-gray-600">
          {ogNetworkDatas[ogNetworkDatas.length - 1]["used_memory"]}/{ogNetworkDatas[ogNetworkDatas.length - 1]["total_memory"]} MB
        </p>
      )}
    </div>
    <div class="bg-white rounded-lg shadow-md p-6">
      <h2 class="text-lg font-semibold mb-4">Average Load time for 1 minute</h2>
      {client_selection !== "" && (
        <p class="text-gray-600">
          {ogNetworkDatas[ogNetworkDatas.length - 1]["avg_load_1_min"]} Seconds
        </p>
      )}
    </div>
    <div class="bg-white rounded-lg shadow-md p-6">
    ...
    {device === "central" &&(
          <div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-semibold mb-4">Is React Server running</h2>
                <p className="text-gray-600">
                {ogNetworkDatas[ogNetworkDatas.length - 1]["react_server"]=== true ? "Yes" : "No"}
                </p>
            </div>

            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-semibold mb-4">Is Firebase Collector running</h2>
                <p className="text-gray-600">
                {ogNetworkDatas[ogNetworkDatas.length - 1]["firebase_collector"]=== true ? "Yes" : "No"}
                </p>
            </div>

            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-semibold mb-4">Is Kiosk Display running</h2>
                <p className="text-gray-600">
                {ogNetworkDatas[ogNetworkDatas.length - 1]["kiosk_display"]=== true ? "Yes" : "No"}
                </p>
            </div>
